# -*- coding: utf-8 -*-
"""Feeder analysis tab — per-match drill-down + season aggregate."""
...
from analysis import feeder_analysis
...
def render_per_log(r: dict) -> None:
    ...
    if any(m.get("stats") for m in r.get("hoot_motors", [])):
        st.markdown(
            "**Per-motor telemetry** (from paired canivore-bus hoot — "
            "DeviceTemp, SupplyCurrent, TorqueCurrent)"
        )
        st.dataframe(_hoot_motor_df(r), hide_index=True, width="content")

    st.markdown("**Time in each Feeder State**")
    rows = []
    for st_name in feeder_analysis.STATES:
    ...
    raw_report(capture_text(feeder_analysis.print_per_log_report, r))
...
def render_combined(results: list[dict]) -> None:
    ...
    raw_report(capture_text(feeder_analysis.print_combined_analysis, results),
...
def render(results: list[dict]) -> None:
    ...
    with tab_match:
        chosen = per_match_picker(results, key="feeder_match_pick")
