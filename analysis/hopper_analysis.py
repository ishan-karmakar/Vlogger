# -*- coding: utf-8 -*-
"""
Hopper energy & performance analysis for FRC Team Valor 6800.

Two motors: Left Hopper Motor, Right Hopper Motor (TalonFX 14 + 15, rio).
Cycle model: Hopper State transitions (DISABLED <-> SHOOTING / OUTTAKING).
Power     : |Out Volt| * |Stator Current| per motor, summed.

Note: NT publishes Stator Current but NOT Supply Current for hopper motors;
...
Usage:
    python hopper_analysis.py                          # default log
    python hopper_analysis.py logs/                    # directory (recursive)
    python hopper_analysis.py logs/*.wpilog            # glob
    python hopper_analysis.py -j 8 logs/               # 8 workers
    python hopper_analysis.py --no-file logs/          # terminal only
"""
...
# Note: no `Supply Current` in this regex — hopper motors don't publish it on NT
# (they would log it via hoot, which we read separately).
HOPPER_REGEX = (
    r"NT:/SmartDashboard/Intake/("
    r"Hopper State"
    r"|(Left|Right) Hopper Motor/(Speed|Stator Current|Out Volt|reqSpeed)"
    r")"
)
REGEX = HOPPER_REGEX  # canonical alias

L_SPEED    = "NT:/SmartDashboard/Intake/Left Hopper Motor/Speed"
L_STATOR   = "NT:/SmartDashboard/Intake/Left Hopper Motor/Stator Current"
L_VOLTAGE  = "NT:/SmartDashboard/Intake/Left Hopper Motor/Out Volt"
L_REQSPEED = "NT:/SmartDashboard/Intake/Left Hopper Motor/reqSpeed"
R_SPEED    = "NT:/SmartDashboard/Intake/Right Hopper Motor/Speed"
R_STATOR   = "NT:/SmartDashboard/Intake/Right Hopper Motor/Stator Current"
R_VOLTAGE  = "NT:/SmartDashboard/Intake/Right Hopper Motor/Out Volt"
R_REQSPEED = "NT:/SmartDashboard/Intake/Right Hopper Motor/reqSpeed"
STATE_PATH = "NT:/SmartDashboard/Intake/Hopper State"

# Hoot CAN-ID mapping for the 2 hopper motors. Both live on the **rio** bus
# (alongside the intake roller motors at IDs 12-13), unlike feeder/flywheel/
# drivetrain which sit on canivore. Bus selection drives which paired hoot
# file we read — see find_paired_hoots(bus="rio") below.
CAN_HOPPER = (
    ("Left",  14),
    ("Right", 15),
)
HOOT_REGEX = _hoot.hoot_regex(cid for (_, cid) in CAN_HOPPER)
...
# State-machine values published as `Intake/Hopper State`. Anything that isn't
...
def load_series(log_path):
    ...
    src = vlogger.get_source(url, HOPPER_REGEX)
    _hoot.load_into_raw(raw, src)
    hoot_files_used = _hoot.attach_paired_hoots(raw, log_path, HOOT_REGEX, bus="rio")
...
def find_state_windows(state_pts, target_state):
    ...
    around the shared helper using hopper's MIN_CYCLE_SECS and the
    "close on any non-target" convention (intake / hopper / hopper share)."""
...
def analyze_log(log_path):
    """Run per-log hopper analysis, returning a result dict or None when
    required signals are missing (typically: motor speed series, or no
    Hopper State logged at all)."""
...
def print_per_log_report(r):
    ...
    if r.get("max_motor_temp_c") is not None:
        print(f"  Peak motor temp         : {r['max_motor_temp_c']:.1f} °C  "
              f"(across both hopper motors, from hoot)")
    ...
    print()
    print(f"  Time in each Hopper State:")
...
def print_combined_analysis(results):
    ...
    print()
    print(SEP)
    print(f"  HOPPER ANALYSIS  --  SEASON SUMMARY  ({n_matches} match{'es' if n_matches != 1 else ''})")
    print(SEP)
    ...
    print(f"    Hopper energy        : {total_energy_J/1000:.2f} kJ")
...
def parse_cli(argv):
    ...
    summary_out = os.path.join(reports_dir, "hopper_summary.md")
    matches_out = os.path.join(reports_dir, "hopper_matches.md")
    write_file  = True
    workers     = None
    positional  = []
    i = 0
...
def main():
    ...
    with contextlib.redirect_stdout(matches_buf):
        ...
        for p in paths:
            print(f"  - {p}")
        for p in failed:
            print(f"\nWARNING: required hopper signals missing in {p}.")
        for r in results:
    ...
    if write_file:
        ...
        write_markdown_report("Hopper Analysis - Per-Match Breakdown",
        ...
        write_markdown_report("Hopper Analysis - Season Summary",
